...
{
    // trida definujici strategii pro praveho hrace
    public class RightPlayerStrategy : Strategy
    {
        int lastRule = -1;
        Graph graph;
        ZOrder zorder;
        ...
        public RightPlayerStrategy(GameSetting gameSetting)
            : base(gameSetting)
        {
        ...
        public override void AdaptStrategy(List<GridRule> adaptedRules)
        {
        ...
        //            size = gStrategy.GridToReal(rule.Oppnt[0], gameSetting).DistanceFrom(storage.LeftRobots[i[0]].Position) * ((double)p[i[0]] / 100) + gStrategy.GridToReal(rule.Oppnt[1], gameSetting).DistanceFrom(storage.LeftRobots[i[1]].Position) * ((double)p[i[1]] / 100) + gStrategy.GridToReal(rule.Oppnt[2], gameSetting).DistanceFrom(storage.LeftRobots[i[2]].Position) * ((double)p[i[2]] / 100) + gStrategy.GridToReal(rule.Oppnt[3], gameSetting).DistanceFrom(storage.LeftRobots[i[3]].Position) * ((double)p[i[3]] / 100);
        ...
        //            myRuleSize = gStrategy.GridToReal(rule.Mine[0], gameSetting).DistanceFrom(storage.RightRobots[i[0]].Position) * ((double)p[i[0]] / 100) + gStrategy.GridToReal(rule.Mine[1], gameSetting).DistanceFrom(storage.RightRobots[i[1]].Position) * ((double)p[i[1]] / 100) + gStrategy.GridToReal(rule.Mine[2], gameSetting).DistanceFrom(storage.RightRobots[i[2]].Position) * ((double)p[i[2]] / 100) + gStrategy.GridToReal(rule.Mine[3], gameSetting).DistanceFrom(storage.RightRobots[i[3]].Position) * ((double)p[i[3]] / 100);
        ...
        //    for (int i = 0; i < 4; i++)
        //    {
        //        if (gStrategy.RealToGrid(storage.RightRobots[ruleIndex[i]].Position, gameSetting) == currentRule.Move[i])
        //        {
        ...
        //    foreach (Robot r in storage.RightRobots)
        //    {
        //        if (r.Position.DistanceFrom(storage.Ball.Position) < min && r != storage.RightRobots[4] && r.Position.DistanceFrom(storage.Ball.Position) < gStrategy.AttackerMin * ((gameSetting.AREA_X + gameSetting.AREA_NET_X * 2) / gStrategy.Size.X))
        //        {
        //            attacker = r;
