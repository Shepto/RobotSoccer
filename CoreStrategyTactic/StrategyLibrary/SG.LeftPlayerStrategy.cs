...
namespace StrategyLibrary
{
    ...
    public class LeftPlayerStrategy : Strategy
    {
        ...
        {
            SetGoalkeeperStrategy(storage.LeftRobots[gameSetting.NUMBER_OF_ROBOTS - 1], storage);
            //aa
            // strategie
            ...
            if (currentRule != null)
            {
                ...
                //    storage.LeftRobots[ruleIndex[i]].PositionMove = gStrategy.GridToReal(currentRule.Move[i], gameSetting);
                int[] mapping = MapRobotsToMoveTo(storage, currentRule);
                for (int i = 0; i < gameSetting.NUMBER_OF_ROBOTS - 1; i++)
                    storage.LeftRobots[mapping[i]].PositionMove = gStrategy.GridToReal(currentRule.Move[i], gameSetting);
            }
        ...
        private int[] MapRobotsToMoveTo(Storage storage, GridRule rule)
        {
            ...
            {
                double moveRuleSize = gStrategy.GridToReal(rule.Move[0], gameSetting).DistanceFrom(storage.LeftRobots[i[0]].Position)
                    + gStrategy.GridToReal(rule.Move[1], gameSetting).DistanceFrom(storage.LeftRobots[i[1]].Position)
                    + gStrategy.GridToReal(rule.Move[2], gameSetting).DistanceFrom(storage.LeftRobots[i[2]].Position)
                    + gStrategy.GridToReal(rule.Move[3], gameSetting).DistanceFrom(storage.LeftRobots[i[3]].Position);
                if (moveRuleSize < min)
                {
                    min = moveRuleSize;
                    bestPerm = i;
