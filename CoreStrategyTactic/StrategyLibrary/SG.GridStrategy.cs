...
{
    //public interface IGridStrategyHolder
    //{
    //    GridStrategy GStrategy
    //    {
    //        get;
    //    }
    //}
    // trida predstavujici strategii tvorenou vice pravidly a nekolika dalsimi informacemi
    public class GridStrategy
    {
        ...
        public Vector2D GridToReal(Vector2D gridPoint, GameSetting gameSetting)
        {
            ...
            //return new Vector2D(
            //    (gridPoint.X - 1) * ((gameSetting.AREA_X + 2 * gameSetting.AREA_NET_X) / size.X) - (gameSetting.AREA_X + 2 * gameSetting.AREA_NET_X) / 2 + (gameSetting.AREA_X + 2 * gameSetting.AREA_NET_X) / (2 * size.X),
            //    (gridPoint.Y - 1) * (gameSetting.AREA_Y / size.Y) - gameSetting.AREA_Y / 2 + gameSetting.AREA_Y / (2 * size.Y));
        }
        ...
        public Vector2D RealToGrid(Vector2D realPoint, GameSetting gameSetting)
        {
            ...
            //return new Vector2D(
            //    (int)((realPoint.X + gameSetting.AREA_X / 2 + gameSetting.AREA_NET_X) / ((gameSetting.AREA_X + 2 * gameSetting.AREA_NET_X) / size.X)) + 1,
            //    (int)((realPoint.Y + gameSetting.AREA_Y / 2) / ((gameSetting.AREA_Y) / size.Y)) + 1);
        }
